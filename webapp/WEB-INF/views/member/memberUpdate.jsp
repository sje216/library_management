...
<title>일반 회원 정보 수정 페이지</title>
<!--일반 회원이 개인정보를 확인,수정 하고 싶을때 확인하는곳-->
</head>
<body class="mainbody">
	...
	<main>
		<div class="EAdiv">
			<h2>회원 정보</h2>
			<form:form commandName="updateCommand" action="changeMember" method="post">
				<p>회원 번호</p>
				<input class="ATinput" type="name" name="memberNum" value="${member.memberNum}" readonly="readonly">
				<p>아이디</p>
				<input class="ATinput" type="name" name="id" value="${member.id}" readonly="readonly">
				<p>이름</p>
				...
				<input class="ASinput ASinputred" type="reset" value="탈퇴하기" onclick="secession()">
	...
	var testV =1;
		 function secession() {
			...
			while(testV<3){
				...
				if(${member.pw}==pw){
					alert('탈퇴가 완료되었습니다.');
					window.location.replace('secession_member');
					break;
				}
				testV+=1;
			}
