				...
				<div class="FFdiv">
					...
					<thead>
						<tr class=BTT>
							...
							<th class="BTHTT">조회수</th>
						</tr>
						<c:forEach var="h" items="${hopes}" varStatus="n">
						<tr>
						<th>${h.num}</th>
						<th>${h.id}</th>
						<th><a href="hope_detail?num=${h.num}">${h.title}</a></th>
						<th>${h.datetime}</th>
						<th>${h.viewCnt}</th>
						</tr>
						</c:forEach>
					...
					<c:if test="${fullPage != null}">
						<c:choose>
								...
								<c:if test="${(section)*100 < fullPage}">
									...
									<c:forEach var="page" begin="1" end="10" step="1">
										...
										<c:if test="${section > 1 && page == 1}">
											<a
												href="<c:url value='/hopelist?section=${section-1}&pageNum=${10}'/>">
												pre </a>
										</c:if>
										<a
											href="<c:url value='/hopelist?section=${section}&pageNum=${page}'/>">${(section-1)*10+page}
										</a>
										<c:if test="${page == 10}">
											<a
												href="<c:url value='/hopelist?section=${section+1}&pageNum=${1}'/>">next
											</a>
										</c:if>
									</c:forEach>
								</c:if>
							...
							<c:when test="${fullPage == 100}">
								...
								<c:forEach var="page" begin="1" end="10" step="1">
									<a
										href="<c:url value='/hopelist?section=${section}&pageNum=${page}'/>">${page}</a>
								</c:forEach>
							</c:when>
							<c:when test="${fullPage < 100}">
								<!-- 1 2 3 4 5 6 7 -->
